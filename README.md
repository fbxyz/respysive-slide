...
```
```
Note that most of Content methods and add_col accept **kwargs for css style. Just replace `-` by `_` :
...
```
```
divclass=['fragment'] is used to pass multiple class, like reveal "fragment"
For the add_heading() method, Fontawesome icons can be added.
```
...
It is **highly recommended** to set chart's width and height manually (900*500 in the two examples)
